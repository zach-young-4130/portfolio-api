  ...
  module V1
    class CommunityItemsController < BaseController
      ...
      def create
        ...
        if item.save
          render_created(community_item: CommunityItemBlueprint.render_as_hash(item))
        else
      ...
      def community_item_params
        params.expect(community_item: %i[title description url role year position published])
      end
    end
  end
end
